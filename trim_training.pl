...
my $read_path_triticum = "/jic/scratch/groups/Philippa-Borrill/raw_data/example_RNA_seq/fastq"; # Define the directory with the files
my $output_dir = "/hpc-home/zik25cof/rna-training/Trimm/results"; # Define the directory where trimmed files were will be stored (output)

## Secondly we specified the input of the samples. Previously, we need to make a .txt file with all the samples (see examples):
my $input_list_dir = "/hpc-home/zik25cof/rna-training/Trimm"; # Path for the folder where the list of samples is stored
my $input_for_trim = "/hpc-home/zik25cof/rna-training/Trimm/input_for_trim_training.txt"; # Path for the list.txt of samples itself
...
chdir("$input_list_dir") or die "coulnd't open the input directory"; # Indicate where the input directory defined above is located
...
#SBATCH -c 4
#SBATCH --mem=120000
#SBATCH -o /hpc-home/zik25cof/rna-training/Trimm/results/%x.%N.%j.out
#SBATCH -e /hpc-home/zik25cof/rna-training/Trimm/results/%x.%N.%j.err
...
SLURM
 ...
 my $tmp_file = "$output_dir/tmp/trim.$sample"; # Previously, a folder with the name "tmp" needs to be created inside the output folder
  ...
  print SLURM "source package 50fcf79b-73a3-4f94-9553-5ed917823423\n";
	...
	print SLURM "trimmomatic PE -phred33 -threads 4 $f1 $f2 $output_dir/$sample"."_1.paired.fq.gz $output_dir/$sample"."_1.unpaired.fq.gz $output_dir/$sample"."_2.paired.fq.gz $output_dir/$sample"."_2.unpaired.fq.gz ILLUMINACLIP:$adapter_dir/TruSeq3-PE.fa:2:30:10 LEADING:3 SLIDINGWINDOW:4:15 MINLEN:80\n";
